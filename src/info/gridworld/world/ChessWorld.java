...
public class ChessWorld<E extends ChessPiece> extends World<E> {
    ...
    private Object[] optionValues = {"Restart", "View Board","Quit"};
	/**
	...
	//restart game
	public void stop(){
		chessGame = new ChessGame(1800000);//30 minute chess timer
		
		super.setGrid((Grid<E>)chessGame.getChessBoard());
		locOfPieceSelected = null;
		isPieceSelected = false;
		highLightedSpaces = new ArrayList<Location>();
	...
	/**
	 * A method called at game endings when checkmate or draw is called, bring up a window for player to interact with
	 * @return the next option
	 */
	public Object showOption(String message) {
		Object selectedValue = JOptionPane.showInputDialog(null, message, "Input", 
				JOptionPane.INFORMATION_MESSAGE, null, optionValues, optionValues[0]);
		return selectedValue;
	}
	public void draw() {
		String message = "A Draw! Try again?";
		chessGame.endGame();
		this.endGame(message);
	}
	public void disable(Object option) {
	...
	public void endGame(String message) {
		Object option = showOption(message);
		if(option == "Restart"){
			//this.stop();
			chessGame.restartGame(1800000);
			setMessage("White player's turn.");
		} else if (option == "Quit") {
			System.exit(0);
		} else {
